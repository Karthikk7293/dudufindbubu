...
function interact(){
  ...
  if(nearby.id==='bubu'){
    if(state.completed){audio.chirp(true);speak(['“Can we stay here a little longer?” ♡','“You’re my favorite adventure, Dudu.”','“Best. Birthday. Ever.”'][Math.floor(Math.random()*3)],'bubu');return;}
...
function animate(time){
  ...
  if(playing&&!paused&&!world.cinematic){
    ...
    if(dx||dz){
      ...
      const targetAngle=Math.atan2(dx,dz),diff=Math.atan2(Math.sin(targetAngle-world.dudu.rotation.y),Math.cos(targetAngle-world.dudu.rotation.y));world.dudu.rotation.y+=diff*Math.min(1,dt*12);
