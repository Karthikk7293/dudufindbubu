...
export class Companion {
  constructor(position, obstacles, layout) {
    this.position={...position};this.obstacles=obstacles;this.layout=layout;this.route=[];this.replan=0;
  }
  update(dt, target) {
    ...
    if(this.replan<=0){this.route=findPath(this.position,target,this.obstacles,this.layout);this.replan=.65;}
    ...
    while(this.route.length&&remaining>0){
      ...
      moveWithCollisions(this.position,dx/length*step,dz/length*step,this.obstacles,this.layout);remaining-=step;
