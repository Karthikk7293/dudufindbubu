...
try{
  ...
  assert.equal(await page.locator('[data-place="moonwatch"]').isDisabled(),true);
  ...
  await page.waitForFunction(()=>window.__dudu.snapshot().route>0);
  await page.locator('#guide-button').click();
  assert.equal((await snap()).route,0);
  await page.locator('#return-birthday').click();
  assert.notEqual((await snap()).guidance.id,'bridge');
  console.log('Journal detour and cancellation passed');
