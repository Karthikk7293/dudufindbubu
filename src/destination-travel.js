...
import { DESTINATIONS, destinationById, freshTravel, rememberLandmark, collectTravelGift } from './destinations.js';
import { Companion } from './companion.js';
import { animateBearPose, turnBear } from './bear-motion.js';
import { lookAtBearTarget, gazeAtBearTarget } from './bear-expression.js';

export class DestinationTravel {
  constructor(world){
    this.world=world;this.current=DESTINATIONS[0];this.log=freshTravel();this.time=0;this.active=null;this.friendMoment=null;
  ...
  async visit(id){
    ...
    world.story='exploring';world.bubu.visible=true;
    world.companion=new Companion({x:world.state.position.x+1.7,z:world.state.position.z},next.obstacles,next.layout);
  ...
  leaveDestination(){
    this.log.positions[this.current.id]={...this.world.state.position};this.friendMoment=null;this.active?.dispose();this.active=null;
  }
  returnHome(){
    if(!this.away)return;
  ...
  nearbyGift(){return this.active?.nearestGift(this.world.state.position,this.log.gifts)??null;}
  collectGift(site){
    const world=this.world;
    if(!collectTravelGift(this.log,this.current.id,site,world.state.position))return false;
    world.reactions.emit(world.dudu.position,world.reducedMotion);world.dudu.userData.expression.react('delighted',2.2);return true;
  }
  nearbyFriend(){return this.active?.nearbyFriend(this.world.state.position,this.world.nightBlend)??null;}
  greetAnimal(animal){
    const world=this.world;
    if(!animal?.brain.greet(world.state.position,world.nightBlend))return false;
    this.friendMoment={animal,remaining:4};world.dudu.userData.expression.react('greet',2.6);
    world.dudu.rotation.y=Math.atan2(animal.group.position.x-world.dudu.position.x,animal.group.position.z-world.dudu.position.z);
    world.reactions.emit(animal.group.position,world.reducedMotion);return true;
  }
  giftCount(){return this.log.gifts.filter(key=>key.startsWith(this.current.id+'/')).length;}
  update(dt,moving,paused,animateFace){
    ...
    if(this.friendMoment){
      ...
      if(moving||paused||this.friendMoment.remaining<=0)this.friendMoment=null;
    }
    // Dudu looks at whatever is closest to hand: a friend, a gift, then Bubu.
    const friend=this.friendMoment?.animal,gift=this.nearbyGift();
    const near=Math.hypot(world.bubu.position.x-pos.x,world.bubu.position.z-pos.z)<4;
    const duduTarget=friend?{x:friend.group.position.x,z:friend.group.position.z,y:friend.group.position.y+.45}
      :gift?{x:gift.x,z:gift.z,y:region.height(gift.x,gift.z)+.9}
      :near&&world.bubu.visible?{x:world.bubu.position.x,z:world.bubu.position.z,y:world.bubu.position.y+1.25}:null;
    const bubuTarget=near?{x:world.dudu.position.x,z:world.dudu.position.z,y:world.dudu.position.y+1.25}:null;
    for(const [bear,target] of [[world.dudu,duduTarget],[world.bubu,bubuTarget]])
      bear.userData.expression.update(dt,{mood:bear.userData.restMood||'calm',look:lookAtBearTarget(bear,target),gaze:gazeAtBearTarget(bear,target),scripted:false},world.reducedMotion);
    ...
    region.update(dt,n,rain,world.reducedMotion,this.log.memories,pos,{gifts:this.log.gifts,walkable:(x,z)=>world.walkable(x,z),paused});
  ...
  drawMap(ctx,w,h){
    const scale=Math.min(w,h)/88,cx=w/2,cy=h/2,point=(x,z)=>[cx+x*scale,cy+z*scale];
    ctx.fillStyle=`#${this.current.ground.toString(16).padStart(6,'0')}`;ctx.beginPath();ctx.arc(cx,cy,40*scale,0,Math.PI*2);ctx.fill();
    if(this.current.id==='beach'){ctx.fillStyle='#77b8c3';ctx.fillRect(cx-40*scale,cy-40*scale,80*scale,30.5*scale);}
    ...
    // Only gifts already in the bag are recorded; the rest stay to be found.
    for(const site of this.active.gifts){
      if(!this.log.gifts.includes(`${this.current.id}/${site.item.id}`))continue;
      const [x,y]=point(site.x,site.z);ctx.fillStyle='#c39a5f';ctx.fillRect(x-scale*.8,y-scale*.8,scale*1.6,scale*1.6);
    }
  ...
  snapshot(){return {id:this.current.id,name:this.current.name,visited:[...this.log.visited],memories:[...this.log.memories],gifts:[...this.log.gifts],landmarks:this.current.landmarks,giftSites:this.active?.gifts.map(site=>({id:site.item.id,x:site.x,z:site.z}))??[],animals:this.active?.animals.map(a=>({kind:a.kind,x:a.group.position.x,z:a.group.position.z,mode:a.brain.mode}))??[],sceneCount:this.active?1:0,forestVisible:this.forestRoot.visible,homePosition:this.home?.position,animationTime:this.active?.time};}
}
