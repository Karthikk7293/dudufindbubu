...
const errors=[];
try{
  const page=await browser.newPage({viewport:{width:1000,height:600},deviceScaleFactor:1});page.setDefaultTimeout(120000);page.on('pageerror',e=>errors.push(e.message));
  ...
  await page.locator('#play-pause').click();const paused=(await snap()).partyTime;await page.waitForTimeout(200);assert.equal((await snap()).partyTime,paused);
